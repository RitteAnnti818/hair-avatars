    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        for _sub_step in range(grad_accum_steps):
            ...
            if gaussians.binding != None:
                ...
                if gaussians.enable_hair_strands and gaussians._strand_delta_static is not None:
                    ...
                    if gaussians.strand_temporal_mode == 'smooth' and opt.lambda_strand_temporal_smooth != 0:
                        losses['strand_temporal_smooth'] = gaussians.compute_strand_temporal_smoothness_loss(
                            opt.threshold_strand_temporal_smooth
                        ) * opt.lambda_strand_temporal_smooth
                    if gaussians.strand_temporal_gate is not None and opt.lambda_strand_temporal_gate_l2 != 0:
                        gate_l2 = sum(p.norm() for p in gaussians.strand_temporal_gate.parameters())
                        losses['strand_temporal_gate_l2'] = gate_l2 * opt.lambda_strand_temporal_gate_l2
