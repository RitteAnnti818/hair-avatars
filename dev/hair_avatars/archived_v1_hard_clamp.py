# Archived: inextensible-chain v1 (hard norm-clamp), superseded by v2.
#
# This version was deleted from scene/flame_gaussian_model.py when v2 replaced it, and no separate
# git commit ever captured it -- kept here for reference. See next_steps_plan.md for the live design.
#
# Result: static-only (--disable_strand_dynamic) on 306/302, FREE sequence, vs. the free-vector
# static-only baseline (+0.281dB / +0.014dB precise-hair-crop delta-PSNR):
#
#   cap=0.3 (= threshold_strand_static):  306 +0.050dB   302 -0.027dB
#   cap=0.5 (relaxed):                    306 +0.045dB   302 -0.051dB
#
# Both cap values underperformed the free-vector baseline on both subjects; relaxing the cap didn't
# recover the gain, which ruled out "cap value too tight" as the explanation. The gradient analysis
# below shows why: this clamp entangles magnitude and direction through a single raw vector's norm,
# so the direction gradient decays toward zero once the raw vector drifts past the cap. v2
# (magnitude and direction as separate parameters) was built to remove that entanglement -- but
# landed at basically the same result anyway (+0.054dB / -0.025dB), which means the real bottleneck
# was never the gradient entanglement in the first place: capping a link's magnitude at ~0.3, by any
# mechanism, just trades away capacity the free-vector model was using.
#
...
# save_ply/load_ply, or the loss section in train.py.

import torch


class FlameGaussianModelV1Snippet:
    """Not a real class -- these two methods lived on FlameGaussianModel, kept together here for
    reference. `self` below refers to a FlameGaussianModel instance with enable_hair_strands=True."""

    @staticmethod
    def _clamp_norm(v, cap, eps=1e-8):
        """Rescale v so ||v|| <= cap exactly, leaving v unchanged when it's already inside the cap.
        Unlike a ReLU-threshold penalty (which only discourages ||v|| > cap via a loss gradient, and
        can still be outweighed by a stronger competing gradient), this makes ||v|| > cap unreachable
        by construction -- no parameter value can produce a v whose clamped output exceeds the cap."""
        norm = v.norm(dim=-1, keepdim=True)
        return v * (cap / norm.clamp(min=cap + eps))

    def _update_strand_delta_cumsum(self, timestep):
        """Accumulate the per-link learned offset delta_j = static_j + g(m(t))*dynamic_j(t) along
        each strand's chain: Delta_j = Delta_{j-1} + w_j * delta_j, Delta_0 = 0 (root stays perfectly
...
# --- gradient analysis that ruled out a smoother (tanh) variant too, before spending GPU time on it ---
#
...
#   30x       0.033    0.033
#
# Both decay asymptotically as ~cap/n; the soft version is additionally *worse* than hard clamp in
# the below-cap regime (0.924, 0.762 vs. hard clamp's exact 1.0 no-op there). Never trained --
# ruled out by this analysis alone.
