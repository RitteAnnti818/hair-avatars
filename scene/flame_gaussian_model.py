...
from roma import rotmat_to_unitquat, quat_xyzw_to_wxyz, rotvec_to_unitquat, quat_product, quat_action
...
class FlameGaussianModel(GaussianModel):
    def __init__(self, sh_degree : int, disable_flame_static_offset=False, not_finetune_flame_params=False, n_shape=300, n_expr=100,
                 ...
                 enable_inextensible_chain=False, inextensible_static_cap=0.3, inextensible_dynamic_cap=0.05):
        ...
        # HairAvatars: structural inextensibility, v2 (magnitude/direction decoupled).
        # v1 (post-hoc hard-clamp: v * min(1, cap/||v||) applied to a free vector) was tried and
        # failed on 306/302 at two different cap values (0.3: +0.050/-0.027dB, 0.5: +0.045/-0.051dB
        # vs. the free-vector baseline's +0.281/+0.014dB) -- relaxing the cap didn't help, and a
        # gradient analysis showed why: any post-hoc radial clamp/squash (hard or a tanh-based soft
        # version alike) entangles magnitude and direction through a single raw vector's norm, so the
        # DIRECTION gradient decays as raw norm grows past the cap (verified: d(out)/d(raw) -> 0 as
        # ||raw|| -> infinity for both hard and soft variants, nearly identically).
        # v2 decouples them: magnitude m in [0, cap) is its own sigmoid-gated scalar (standard,
        # bounded gate saturation -- same well-understood trade-off as any LSTM/attention gate, not
        # entangled with direction); direction is a rotation of a fixed reference vector (its gradient
        # stays bounded away from zero regardless of how large the rotation parameter grows --
        # verified numerically, unlike a norm-based clamp). ||delta|| < cap always, by construction,
        # in both versions -- v2 changes *how* that's achieved, not *whether*.
        self.enable_inextensible_chain = enable_inextensible_chain
        self.inextensible_static_cap = inextensible_static_cap
        self.inextensible_dynamic_cap = inextensible_dynamic_cap
        self._strand_mag_raw_static = None    # (num_strands, max_chain_len)
        self._strand_dir_aa_static = None     # (num_strands, max_chain_len, 3) axis-angle
        self._strand_mag_raw_dynamic = None   # (num_strands, max_chain_len, num_timesteps)
        self._strand_dir_aa_dynamic = None    # (num_strands, max_chain_len, num_timesteps, 3)
        self._strand_dir_ref = None           # (3,) fixed reference vector the direction rotates
        ...
        if self.flame_param is None:
            ...
            if self.enable_hair_strands and self.face_strand_id is not None:
                if self.enable_inextensible_chain:
                    # v2: magnitude (sigmoid-gated scalar) + direction (axis-angle rotation of a fixed
                    # reference) instead of a free vector. raw_mag initialized to -5 -> sigmoid(-5)~=0.0067,
                    # i.e. m ~= 0.0067*cap (near-zero, safe-by-construction start, but not in the fully
                    # dead flat tail of the sigmoid the way -10 would be, so there's still meaningful
                    # gradient available to grow it from init).
                    self._strand_mag_raw_static = torch.full((self.num_strands, self.max_chain_len), -5.0).cuda()
                    self._strand_dir_aa_static = torch.zeros(self.num_strands, self.max_chain_len, 3).cuda()
                    if not self.disable_strand_dynamic:
                        self._strand_mag_raw_dynamic = torch.full(
                            (self.num_strands, self.max_chain_len, self.num_timesteps), -5.0
                        ).cuda()
                        self._strand_dir_aa_dynamic = torch.zeros(
                            self.num_strands, self.max_chain_len, self.num_timesteps, 3
                        ).cuda()
                    self._strand_dir_ref = torch.tensor([1.0, 0.0, 0.0]).cuda()
                else:
    ...
    def select_mesh_by_timestep(self, timestep, original=False):
        ...
        if self.enable_hair_strands and (self._strand_delta_static is not None or self._strand_mag_raw_static is not None):
    ...
    def _mag_dir_delta(self, mag_raw, dir_aa, cap):
        """v2 inextensible parameterization: delta = m * R(dir_aa) . e_ref, m = cap*sigmoid(mag_raw)
        in [0,cap) always (own scalar, own gradient -- standard bounded gate saturation, not entangled
        with direction), direction = a rotation of the fixed reference vector self._strand_dir_ref
        (gradient stays bounded away from 0 regardless of how large dir_aa grows, verified numerically
        -- unlike a norm-based clamp/squash, where direction gradient decays as raw norm -> infinity).
        ||delta|| < cap always, by construction."""
        m = cap * torch.sigmoid(mag_raw)  # (..., ) no trailing 3
        q = rotvec_to_unitquat(dir_aa)     # (..., 4) xyzw
        # quat_action needs the reference vector broadcast to q's batch shape explicitly (roma's
        # internal quat_product requires matching ndim, not just broadcastable sizes)
        e_ref = self._strand_dir_ref.view(*([1] * (q.dim() - 1)), 3).expand(*q.shape[:-1], 3)
        return m.unsqueeze(-1) * quat_action(q, e_ref)  # (..., 3)
    ...
    def _update_strand_delta_cumsum(self, timestep):
        ...
        enable_inextensible_chain (opt-in, v2): static_j and dynamic_j(t) are each produced by
        _mag_dir_delta instead of being free vectors -- see that method and the design note in
        __init__ for why (v1's post-hoc norm-clamp was tried and failed twice, root-caused to a
        magnitude/direction gradient entanglement that v2 removes)."""
        if self.enable_inextensible_chain:
            static_t = self._mag_dir_delta(self._strand_mag_raw_static, self._strand_dir_aa_static,
                                            self.inextensible_static_cap)
        else:
            static_t = self._strand_delta_static
        delta_t = static_t
        if self.enable_inextensible_chain:
            if self._strand_mag_raw_dynamic is not None:
                dynamic_t = self._mag_dir_delta(self._strand_mag_raw_dynamic[:, :, timestep],
                                                 self._strand_dir_aa_dynamic[:, :, timestep],
                                                 self.inextensible_dynamic_cap)
        ...
        elif self._strand_delta_dynamic is not None:
    ...
    def training_setup(self, training_args):
        ...
        if self.enable_hair_strands and self.enable_inextensible_chain and self._strand_mag_raw_static is not None:
            self._strand_mag_raw_static.requires_grad_(True)
            self._strand_dir_aa_static.requires_grad_(True)
            params = [self._strand_mag_raw_static, self._strand_dir_aa_static]
            if self._strand_mag_raw_dynamic is not None:
                self._strand_mag_raw_dynamic.requires_grad_(True)
                self._strand_dir_aa_dynamic.requires_grad_(True)
                params += [self._strand_mag_raw_dynamic, self._strand_dir_aa_dynamic]
            param_strand = {'params': params, 'lr': training_args.strand_delta_lr, "name": "strand_delta_v2"}
            self.optimizer.add_param_group(param_strand)
        elif self.enable_hair_strands and self._strand_delta_static is not None:
    ...
    def save_ply(self, path):
        ...
        if self.enable_hair_strands and self.enable_inextensible_chain and self._strand_mag_raw_static is not None:
            ...
            strand_data = {
                "mag_raw_static": self._strand_mag_raw_static.detach().cpu().numpy(),
                "dir_aa_static": self._strand_dir_aa_static.detach().cpu().numpy(),
            }
            if self._strand_mag_raw_dynamic is not None:
                strand_data["mag_raw_dynamic"] = self._strand_mag_raw_dynamic.detach().cpu().numpy()
                strand_data["dir_aa_dynamic"] = self._strand_dir_aa_dynamic.detach().cpu().numpy()
    ...
    def load_ply(self, path, **kwargs):
        ...
        if self.enable_hair_strands and self.face_strand_id is not None:
            ...
            if strand_path.exists():
                ...
                if self.enable_inextensible_chain and "mag_raw_static" in strand_data:
                    self._strand_mag_raw_static = torch.from_numpy(strand_data["mag_raw_static"]).cuda()
                    self._strand_dir_aa_static = torch.from_numpy(strand_data["dir_aa_static"]).cuda()
                    if "mag_raw_dynamic" in strand_data:
                        self._strand_mag_raw_dynamic = torch.from_numpy(strand_data["mag_raw_dynamic"]).cuda()
                        self._strand_dir_aa_dynamic = torch.from_numpy(strand_data["dir_aa_dynamic"]).cuda()
                    self._strand_dir_ref = torch.tensor([1.0, 0.0, 0.0]).cuda()
                else:
